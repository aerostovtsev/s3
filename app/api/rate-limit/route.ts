...
const rateLimitConfigs: Record<string, { interval: number; limit: number }> = {
  "/api/upload": { interval: 60, limit: 500 },
  "/api/files": { interval: 60, limit: 500 },
  "/api/users": { interval: 60, limit: 60 },
  "/api/auth": { interval: 60, limit: 3 },
...
export async function GET(request: Request) {
  ...
  console.log(
    `Processing request for path: ${path}, identifier: ${identifier}`
  )

  // Используем IP как идентификатор для rate limiting
  ...
  try {
    ...
    console.log(`Current request count for IP ${identifier}: ${requestCount}`)
    ...
    if (requestCount >= config.limit) {
      ...
      const reset = Math.ceil(
        (parseInt(oldestRequest[0]) + config.interval * 1000 - now) / 1000
      )

      console.log(
        `Rate limit exceeded for IP ${identifier}, reset time:`,
        reset
      )
      return NextResponse.json(
        {
    ...
    console.log(`Adding new request for IP ${identifier}...`)
    await redis.zadd(key, now, now.toString())
    // Устанавливаем время жизни для ключа
    await redis.expire(key, config.interval)

    console.log(`Request processed successfully for IP ${identifier}`)
    return NextResponse.json(
      {
        success: true,
